...
class ROI:
    ...
    def findROI(self, frame, marker):
        ...
        top_lx = int(bot_lx + sign * se
        top_rx = int(bot_rx + sign * self.r_beta * slope_14 * l * np.sqrt(1/(1+slope_14**2)))
        oi_beta * l lf.roi_beta * l * np.sqrt(1/(1+slope_14**2)))
        top_ly = int(bot_ly + sign * self.roi* np.sqrt(1/(1+slope_14**2)))
